...
function wimpie_lite_sidebar_layout($classes){
	...
	function wimpie_lite_header_styles_scripts(){
		...
		if ( function_exists( 'the_custom_logo' ) ) {
			...
			if(($header_bg_v)){
				$header_bg_v =   '.home #masthead.site-header { background: url("'.esc_url($header_bg_v).'") no-repeat scroll left top rgba(0, 0, 0, 0); position: relative; z-index: 1;background-size: cover; }';
				echo $header_bg_v;
				echo "\n";
